...
async function handleAdmin(request, url, env) {
  ...
  // GET / POST /__api/ip-country -> 节点备注的国家标注开关与样式
  if (path === '/__api/ip-country') {
...
if (ntEnabled) {
  api('/__api/ip-country').then(function (r) {
  ...
  if (ntSaveBtn) ntSaveBtn.onclick = async function () {
    ...
    try {
      const r = await api('/__api/ip-country', { method: 'POST', body: JSON.stringify({
