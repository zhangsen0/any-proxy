...
- **模板字符串里的转义要写双反斜杠**：整段 `<script>` 通常是被模板字符串拼出来的，写 `'\n'`
  时 JS 解析模板那一步就把它解释成了真实换行，浏览器收到的是「字符串里出现裸换行」
  → `SyntaxError`，整块脚本一行不执行。正确写法 `'\\n'`，参考 `src/admin.js` 的 2050 / 2093 行。
  这类问题服务端**零症状**：HTML 渲染得好好的、接口全是 200、Worker 日志一条没有，
  只有浏览器控制台留了一句 `Invalid or unexpected token`。
- **注释里不要出现反引号**：它会提前结束外层模板字符串，把后面的内容变成表达式求值
  （真实翻车：注释里写了半个 template 标记，浏览器直接报 `script is not defined`）。
- 页面类改动要有**浏览器视角**的证据。服务端检查看不到浏览器侧的运行时错误，
  必要时刻临时挂一个真浏览器工作流（登录 → 打开页面 → 收集 `pageerror`/`console.error`
  → 数一下按钮到底绑了几个 `onclick`）拿证据，查完即删。
...
**跑在真实多实例环境上的检查，判据是「能否稳定复现」而不是「这次红不红」。** `tools/check-live.mjs` 在同一份部署上连跑三次得到 47/2、47/2、49/0：红色的那几项（写进去的主题不在清单里、轮换模式读到 off）全是「写 → 立刻读」型断言，而 Worker 的进程内缓存是 **per-isolate** 的 —— 上一个请求写在 isolate A 的内存里，下一个请求落到一个刚从种子灌完的新 isolate B，读到的自然是旧账；再加上脚本自己的限流测试会留 429 余波打到后面的步骤上。所以线上 **2 项失败 ≠ 有 bug**：先重跑，只有能稳定复现的才值得追。这条不落进文档就等于没写 —— 脚本在打印失败项时会主动提示「可能是单例缓存抖动」（见 `check-live.mjs` 尾部），别把这个提示删掉。
