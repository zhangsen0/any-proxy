...
- 部署域名：`https://proxy.520215.xyz`
- 示例反代链接：`https://proxy.520215.xyz/p/github/zhangsen0/any-proxy`
...
- **DNS 自动优选**：定时测通 CF 泛播候选 IP 后写入 A 记录，频率前端可调（5~1440 分钟，默认 12 小时），客户端零配置。
...
| `PREF_DOMAINS` | 健康检查改 DNS 时解析用的候选域名池（空格分隔）。优先级：GitHub 变量 → Worker 侧 `PREF_DOMAINS`（面板可配）→ 内置兜底 |
| `PROXY_HOST` | 健康检查验证 / 修复的域名（留空回退 `proxy.520215.xyz`） |
...
| GET | `/__api/preferred-candidates` | 免登录 | 从 sub 订阅拉节点 IP（供浏览器测速，不暴露 token） |
...
src/
  ...
  auth.js              登录 / 登出 / 登录页
  dns.js               优选 IP 与 DNS 自动更新（候选池 → 探测 → 写 A 记录 → 自校验回滚）
  util.js              无依赖小工具
migrations/
